  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        // Notifier l'AuthContext via un événement — il gère le logout proprement
...
export const consultationsAPI = {
  ...
  // Catalogue médicaments suggérés pour une maladie
  getCatalogueMedicaments: (maladie: string) =>
    fetchAPI<Array<{
      id: number;
      nom_commercial: string;
      denomination_commune: string | null;
      dosage: string | null;
      forme: string | null;
    ...
    }>>(`/api/consultations/catalogue-medicaments?maladie=${encodeURIComponent(maladie)}`),
};
...
// Construit le suffixe de query string du filtre de période ('' si aucun filtre)
const periodeSuffix = (periode?: PeriodeFilter, sep: '?' | '&' = '?') => {
  const params = new URLSearchParams();
  if (periode?.dateDebut) params.set('date_debut', periode.dateDebut);
  if (periode?.dateFin) params.set('date_fin', periode.dateFin);
...
export const analyticsAPI = {
  // Dashboard KPIs
  getDashboard: (periode?: PeriodeFilter) =>
    fetchAPI<DashboardStats>('/api/analytics/dashboard' + periodeSuffix(periode)),

  // Séries quotidiennes pour la page Analytique
  getSeries: (periode?: PeriodeFilter) =>
    fetchAPI<{
    ...
    }>('/api/analytics/series' + periodeSuffix(periode)),
  ...
  getRecentConsultations: (limit = 10, periode?: PeriodeFilter) =>
    fetchAPI<Consultation[]>(`/api/analytics/consultations/recent?limit=${limit}${periodeSuffix(periode, '&')}`),
...
export const adminAPI = {
  ...
  getModelPerformance: (token: string) =>
    fetchAPIAuth<{
      ...
      message?: string;
    }>(token, '/api/admin/model/performance'),

  // Statistiques d'utilisation de l'IA
  getAIStats: (token: string) =>
    fetchAPIAuth<{
      total_predictions: number;
      avg_confidence_pct: number;
      confidence_distribution: { HIGH: number; MEDIUM: number; LOW: number };
      top_diseases: Array<{ disease: string; count: number }>;
    }>(token, '/api/admin/ai-stats'),

  // Historique des entraînements
  getTrainingHistory: (token: string) =>
    fetchAPIAuth<{
      sessions: Array<{
    ...
    }>(token, '/api/admin/training/history'),
};
